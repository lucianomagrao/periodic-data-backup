...
echo "Starting recovery from $S3_URL to $PATH_BACKUP"
duplicity --progress --progress-rate 5 --force --no-encryption $S3_URL $PATH_BACKUP
...
echo "Starting monitoring $PATH_BACKUP"
while inotifywait -r -e $inotifywait_events $PATH_BACKUP ; do
  echo "Change detected."
  while inotifywait -r -t $WAIT_TIME -e $inotifywait_events $PATH_BACKUP ; do
  	echo "waiting for quiet period.."
  done

  echo "Starting backup"
  duplicity --progress --progress-rate 5 --no-encryption --allow-source-mismatch --full-if-older-than 7D $PATH_BACKUP $S3_URL
